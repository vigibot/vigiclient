 ...
 sockets[serveur].on("clientsrobottx", function(data) {
  ...
  if(!hard.DEVTELEMETRIE) {
   ...
   for(let i = 0; i < conf.TX.COMMANDES1.length / 8; i++)
    rx.commandes1[i] = tx.commandes1[i];
   setRxVals();
   sockets[serveur].emit("serveurrobotrx", {
    timestamp: now,
    data: rx.arrayBuffer
...
function setGpios(n, consigne) {
 ...
 if(pcaId == SYS.UNUSED) {
  for(let i = 0; i < hard.OUTPUTS[n].GPIOS.length; i++) {
   if(etat == SYS.INPUT)
    gpioOutputs[n][i].mode(GPIO.INPUT);
   else
    gpioOutputs[n][i].digitalWrite(etat);
  }
 } else {
  for(let i = 0; i < hard.OUTPUTS[n].GPIOS.length; i++) {
   let gpio = hard.OUTPUTS[n].GPIOS[i];
   if(etat)
...
function setPwms(n, consigne) {
 ...
 if(pcaId == SYS.UNUSED) {
  ...
  for(let i = 0; i < hard.OUTPUTS[n].GPIOS.length; i++)
   gpioOutputs[n][i].pwmWrite(pwm);
 } else {
  pwm = pwm / 100;
  for(let i = 0; i < hard.OUTPUTS[n].GPIOS.length; i++)
   pca9685Driver[pcaId].setDutyCycle(hard.OUTPUTS[n].GPIOS[i], pwm);
 }
}
...
function setPwmPwm(n, consigne) {
 ...
 if(pcaId == SYS.UNUSED) {
  pwm = map(pwm, -100, 100, -255, 255);
  if(pwm < 0) {
   gpioOutputs[n][0].digitalWrite(false);
   gpioOutputs[n][1].pwmWrite(-pwm);
  } else if(pwm > 0) {
   gpioOutputs[n][0].pwmWrite(pwm);
   gpioOutputs[n][1].digitalWrite(false);
  } else {
   gpioOutputs[n][0].digitalWrite(true);
   gpioOutputs[n][1].digitalWrite(true);
  }
 } else {
  let chIn1 = hard.OUTPUTS[n].GPIOS[1];
  let chIn2 = hard.OUTPUTS[n].GPIOS[2];
  pwm = pwm / 100;
  if(pwm < 0) {
   pca9685Driver[pcaId].channelOff(chIn1);
   pca9685Driver[pcaId].setDutyCycle(chIn2, -pwm);
  } else if(pwm > 0) {
   pca9685Driver[pcaId].setDutyCycle(chIn1, pwm);
...
function setPwmDir(n, consigne) {
 ...
 if(pcaId == SYS.UNUSED) {
  ...
  pwm = pwm / 100;
  if(pwm < 0) {
   pca9685Driver[pcaId].channelOff(hard.OUTPUTS[n].GPIOS[1]);
   pwm = -pwm;
  } else
   pca9685Driver[pcaId].channelOn(hard.OUTPUTS[n].GPIOS[1]);
  pca9685Driver[pcaId].setDutyCycle(hard.OUTPUTS[n].GPIOS[0], pwm);
 }
}
...
function setPwmDirDir(n, consigne) {
 ...
 if(pcaId == SYS.UNUSED) {
  ...
  if(pwm < 0) {
   gpioOutputs[n][1].digitalWrite(false);
   gpioOutputs[n][2].digitalWrite(true);
   pwm = -pwm;
  } else if(pwm > 0) {
   gpioOutputs[n][1].digitalWrite(true);
   gpioOutputs[n][2].digitalWrite(false);
  } else {
   gpioOutputs[n][1].digitalWrite(true);
   gpioOutputs[n][2].digitalWrite(true);
  }
 ...
 if(running) {
  ...
  for(let i = 0; i < hard.OUTPUTS.length; i++) {
   ...
   if(pcaId == SYS.UNUSED) {
    ...
    for(let j = 0; j < hard.OUTPUTS[i].GPIOS.length; j++) {
     let sleepMode = hard.OUTPUTS[i].SLEEPMODES[j];
     let gpio = hard.OUTPUTS[i].GPIOS[j];
     if(sleepMode != "None") {
      if(sleepMode == "High")
       pca9685Driver[pcaId].channelOn(gpio);
      else
